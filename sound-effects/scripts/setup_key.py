#!/usr/bin/env python3
"""Store the Replicate API token for the sound-effects skill.
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        if existing:
            ...
            config.setdefault("default_category", "generic")
            config.setdefault("default_duration", 3)
            config.setdefault("default_steps", 8)
            config.setdefault("default_cfg_scale", 3.5)
            save_config(config)
    ...
    print("You can now use the sound-effects skill.")
