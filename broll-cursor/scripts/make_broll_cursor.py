#!/usr/bin/env python3
"""broll-cursor orchestrator.

Turn a declarative Cursor-agent-chat `session.json` into a short, polished B-roll
clip (the *base layer* of a technical reel): render an animated, human-feeling IDE
agent chat — a typed user prompt + a streaming assistant turn with tool-call rows —
and, with --avatar, composite the talking avatar in a PiP corner (the overlay layer).

Nothing in the session is executed — the assistant only says/does whatever the JSON
declares. This never runs a real agent or shell, so it can't touch your machine.
...
def slugify(s: str, maxlen: int = 48) -> str:
    ...
    return (s[:maxlen].strip("-") or "cursor")


def main() -> int:
    ap = argparse.ArgumentParser(description=__doc__,
                                 formatter_class=argparse.RawDescriptionHelpFormatter)
    ap.add_argument("session", type=Path, help="Cursor agent-chat session JSON (see reference.md)")
    ...
    ap.add_argument("--out-dir", type=Path, default=Path("broll_cursor"))
    ...
    ap.add_argument("--model", default=None, help="override the model pill text")
    ap.add_argument("--theme", default=None, help="override theme (cursor-dark|cursor-light)")
    ...
    entry = {
        ...
        "source": "broll-cursor", "session": str(args.session),
