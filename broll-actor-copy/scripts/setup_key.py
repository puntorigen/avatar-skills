#!/usr/bin/env python3
"""Store the Replicate API token for the broll-actor-copy skill.

Usage:
    python3 setup_key.py YOUR_REPLICATE_API_TOKEN
    python3 setup_key.py --show

The token is shared with the other Replicate-based skills. If any sibling skill
(avatar-talking-video, broll-avatar-camera, voice-clone, gpt-image-2, ...)
already has a valid token configured, this skill discovers it automatically and
you do not need to run this. Run it only to set or refresh the token.
"""
