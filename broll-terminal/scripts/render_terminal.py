#!/usr/bin/env python3
"""Render a terminal session JSON into a base B-roll clip (deterministic).

Loads terminal.html in headless Chromium (high-DPI), injects the timeline built
by timeline.py, then drives `window.seekTo(t)` once per frame and screenshots
each — a pure function of time, so the result is frame-exact and reproducible
(no recording jitter, no dropped frames). The PNG sequence is muxed to H.264.

Nothing from the session is ever executed: the "output" is whatever the session
JSON declares. This never touches a real shell.
"""
...
def render_base(session: dict, out_mp4: Path, *, aspect: str = "9:16",
    ...
    C.log(f"[broll-terminal] render {aspect} {W}x{H} @ {fps}fps  "
    ...
    return {"clip": str(out_mp4), "aspect": aspect, "fps": fps,
            ...
            "frames": nframes, "theme": tl["theme"], "lines": len(tl["lines"])}


def main() -> int:
    ap = argparse.ArgumentParser(description="Render a terminal session JSON to a base clip.")
    ap.add_argument("session", type=Path, help="session JSON (see timeline.py schema)")
    ap.add_argument("-o", "--out", type=Path, default=Path("terminal_base.mp4"))
    ...
    if not TEMPLATE.exists():
        C.die(f"terminal.html template missing at {TEMPLATE}")
