...
- **pdf-remix** — extract a premium reference PDF's **mold** (design system +
  per-page layout archetypes + narrative arc) with Gemini vision, then regenerate
  a brand-new, on-topic multi-page PDF that follows it faithfully: crisp editable
  text (HTML → Chromium) with imagery generated by `asset-generator` (including a
  person from reference photos placed into cover/section poses). The
  `reel-discovery`-for-PDFs: analyze a winning document, then remix it with your
  brand and topic.
